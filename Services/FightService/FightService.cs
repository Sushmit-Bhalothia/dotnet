                 ...
                 while(!defeated){
                    foreach(var attacker in character)
                    {
                        ...
                        else if(!useWeapon && attacker.Skills is not null)
                        {
                            ...
                            response.Data.Log.Add($"{attacker.Name} was unable to attack");
                            continue;
                        }
                        response.Data.Log.Add($"{attacker.Name} attacks {opponentName} using {attackUsed} with {(damage>=0?damage:0)} damage");
